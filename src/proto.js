const { isString, any2Uint8Array, keccak256, hexlifySha256, toHexAddress} = require('./bytes');
const sha256 = hexlifySha256;
const stringToUint8Array = any2Uint8Array;
...
const fromHexString = (hexString) => {
  ...
  return Uint8Array.from(
    Buffer.from(toHexAddress(hexString), 'hex')
  );
};
...
const { ProposalCreateContract, ProposalDeleteContract, ProposalApproveContract } =
  globalThis.TronWebProto;
...
function contractJsonToProtobuf(contract, value, options) {
...
function compareTransaction(transaction, transactionPb) {
...
function txCheckWithArgs(transaction, args, options) {
  const transactionPb = txJsonToPbWithArgs(transaction, args, options);
  return compareTransaction(transaction, transactionPb);
}

function txJsonToPbWithArgs(transaction, args = {}, options = {}) {
  const rawData = transaction["raw_data"];
  const contractJson = rawData.contract[0];
  ...
  // for memo
  if (options.data) {
    rawDataObj.setData(stringToUint8Array(options.data));
  }
